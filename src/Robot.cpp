...
double Robot::autoTurnMotorSpeed = 0.7;
...
void Robot::RobotInit() {
    ...
    // disable safeties on motors - hopefully to eliminate connection dropping
    driveTrain->driverLeftFront->SetSafetyEnabled(false);
    driveTrain->driverLeftBack->SetSafetyEnabled(false);
    driveTrain->driverRightFront->SetSafetyEnabled(false);
    driveTrain->driverRightBack->SetSafetyEnabled(false);

    aircontrolModule.reset(new AircontrolModule());

    cubeIntake.reset(new CubeIntake());
    cubeIntake->leftMotor->SetSafetyEnabled(false);
    cubeIntake->rightMotor->SetSafetyEnabled(false);

    scissorLift.reset(new ScissorLift());
    scissorLift->scissorLiftMotor->SetSafetyEnabled(false);
...
//	frc::SmartDashboard::PutData("Auto Modes", &chooser);
	...
	// auto turn values
	Robot::autoTurnAngle.push_back(22.0);	// left
	Robot::autoTurnAngle.push_back(-20.50);	// right
